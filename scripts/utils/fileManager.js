		...
		if (source[key] instanceof Object && !Array.isArray(source[key])) {
			...
			source[key].forEach((sourceElement) => {
				if (sourceElement instanceof Object && 'id' in sourceElement) {
					const targetElementIndex = target[key].findIndex(
						(targetElement) => targetElement.id === sourceElement.id
					);
					...
					const targetElementIndex = target[key].findIndex(
						(targetElement) => targetElement.domainName === sourceElement.domainName
					);
					if (targetElementIndex !== -1) {
						merge(target[key][targetElementIndex], sourceElement);
					} else {
					...
					const targetElementIndex = target[key].findIndex(
						(targetElement) => targetElement.day === sourceElement.day
					);
					if (targetElementIndex !== -1) {
						merge(target[key][targetElementIndex], sourceElement);
					} else {
...
export { openJsonFile, writeJsonFile, mergeObjectIntoJson, deepMergeObjectIntoJson, merge };
